<template>
        <footer>
            <div class="mainfooter">
            <section>
            <div class="footernavigation">
                <h5 class="footernavigation__title">Information</h5>
                <a class="footernavigation__link" href="#">The Brand</a>
                <a class="footernavigation__link" href="#">Local Store</a>
                <a class="footernavigation__link" href="#">Custom Service</a>
                <a class="footernavigation__link" href="#">Privacy &amp; Cookies</a>
                <a class="footernavigation__link" href="#"> Site map</a>
            </div>
             <div class="footernavigation">
                <h5 class="footernavigation__title">Why buy from us</h5>
                <a class="footernavigation__link" href="#">The Brand</a>
                <a class="footernavigation__link" href="#">Local Store</a>
                <a class="footernavigation__link" href="#">Custom Service</a>
                <a class="footernavigation__link" href="#">Privacy &amp; Cookies</a>
                <a class="footernavigation__link" href="#"> Site map</a>
            </div>
               <div class="footernavigation">
                <h5 class="footernavigation__title">Why buy from us</h5>
                <a class="footernavigation__link" href="#">The Brand</a>
                <a class="footernavigation__link" href="#">Local Store</a>
                <a class="footernavigation__link" href="#">Custom Service</a>
                <a class="footernavigation__link" href="#">Privacy &amp; Cookies</a>
                <a class="footernavigation__link" href="#"> Site map</a>
            </div>
               <div class="footernavigation">
                <h5 class="footernavigation__title">Why buy from us</h5>
                <a class="footernavigation__link" href="#">The Brand</a>
                <a class="footernavigation__link" href="#">Local Store</a>
                <a class="footernavigation__link" href="#">Custom Service</a>
                <a class="footernavigation__link" href="#">Privacy &amp; Cookies</a>
                <a class="footernavigation__link" href="#"> Site map</a>
            </div>

            <div class="promotion">
                <h4 class="promotion__heading"><strong>award winner</strong> <br>Fashion awards 2016</h4>
            </div>
            <div class="socialmediabox">
                     <a class="socialmediabox__link socialmediabox__link--hovered" href="#"><font-awesome-icon icon="heart" size="3x" class="socialmediabox__icon">  </font-awesome-icon></a>
                      <a class="socialmediabox__link socialmediabox__link--hovered" href="#"><font-awesome-icon icon="heart" size="3x" class="socialmediabox__icon">  </font-awesome-icon></a>
                       <a class="socialmediabox__link socialmediabox__link--hovered" href="#"><font-awesome-icon icon="heart" size="3x" class="socialmediabox__icon">  </font-awesome-icon></a>
                        <a class="socialmediabox__link socialmediabox__link--hovered" href="#"><font-awesome-icon icon="heart" size="3x" class="socialmediabox__icon">  </font-awesome-icon></a>

            </div>
    </section>
  
    </div>
      <section class="secondaryfooter">
    <p class="secondaryfooter__link"> &copy; 2016 Avenua Fashion</p>
    <p class="secondaryfooter__link">Design by RobbyDesign.com</p>
    <p class="secondaryfooter__link">Dev by Loremipsum.com</p>
    </section>
    </footer>
</template>

<script>
export default {
  name: "MainFooter"
};
</script>
